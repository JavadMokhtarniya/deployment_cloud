...
def cart_service_test():
    ...
    if code != 200:
        ...
        if code != 200:
            ...
            print(random_user_email+":"+random_user_pass)
    ...
    tokens, code = refresh(tokens["access_token"], tokens["refresh_token"])
    mycart,code=try_add_to_cart(tokens["access_token"],query["id"])
    if(code != 200):
        print("add cart failed : "+str(query)+str(mycart))
    else:
        print("Step[Add To Cart] succeeded")
    # #test modify
    # tokens, code = refresh(tokens["access_token"], tokens["refresh_token"])
    # if(code != 200):
...
cart_service_test()
# get_all_products(tokens["access_token"])
# cart_service_test()